...
- **Range → single transition**: When the picker is already open in range mode (cursor was inside a range expression) and the context changes to a single-date value (e.g., user pastes a single date over the range), the picker is forced to remount so it opens in single mode. The `shouldRemountDatePicker` function compares the previous and new init states; when the mode differs, `showDatePicker` is set to `false` before the rAF re-opens it, ensuring `useState` picks up the fresh `initialMode`. **Tests:** `DatePickerRangeTransition.test.ts` → "shouldRemountDatePicker" suite (4 tests), "full paste-over-range scenario" suite (3 tests)
