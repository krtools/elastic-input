...
- Works inside field groups: `status:(active OR |)` → `FIELD_VALUE`, fieldName=`"status"`. The `findEnclosingFieldGroup` helper scans backwards from the cursor to find an unmatched LPAREN preceded by `FIELD_NAME COLON`, tracking paren depth to skip nested groups. When an unmatched LPAREN is *not* preceded by `FIELD_NAME COLON` (i.e., a plain grouping paren), scanning continues outward through nested parens so that `status:((|))` and deeper nesting still resolve correctly.
- **Tests:** `CursorContext.test.ts` → "returns FIELD_VALUE right after colon", "returns FIELD_VALUE while typing value", "returns FIELD_VALUE after comparison operator", "returns FIELD_VALUE for field:value inside parens", "returns FIELD_VALUE for field:partial inside parens", "field group context" suite (17 tests)
...
For fully custom hint rendering (e.g. multiline rich content with instructions), the `dropdown.renderFieldHint` option accepts a callback `(field: FieldConfig, partial: string) => ReactNode | null`. When provided and returning a non-null value, the custom element replaces the default text hint in the dropdown. This works for **any field** in value position — if no hint suggestion already exists (e.g. the field has enum values or fetchSuggestions), a hint is injected at the top of the suggestion list. Returning `null` falls back to the default behavior. The callback receives the resolved `FieldConfig` (aliases are resolved to the canonical field).
...
### 5.3 Double-Click Word Selection

Double-clicking a word in the editor selects the word without trailing whitespace. The browser's default double-click selects word + trailing space, which causes Delete/Backspace to remove adjacent spacing. The component intercepts `mousedown` with `detail === 2`, calls `preventDefault()`, and computes word boundaries itself using syntax-aware delimiters (whitespace, `:`, `()`, `[]`, `{}`, `"`, operators).
...
### 5.5 Multi-Token Selection Suppresses Dropdown
...
### 5.6 Value Replacement Boundaries
...
Enter with the **date picker open** closes it and submits, the same as with the suggestion dropdown open and nothing selected. (Previously it matched no branch and fell through to the browser, which inserted a newline — e.g. at the end of a typed range, still RANGE context.) Tests: `DatePickerEnter.browser.test.tsx`.
...
**Flip-up anchoring:** the initial flip *decision* uses an estimated dropdown height (suggestion count × 32px, capped at `dropdownMaxHeight`; 350px for the date picker), but the flipped dropdown's *placement* does not. `getDropdownPosition` returns `flipped: true` with `top` set 4px above the caret, and the renderer applies `transform: translateY(-100%)` so the browser pins the dropdown's **bottom edge** to that anchor regardless of actual rendered height. This mirrors the drop-down direction, which anchors the top edge 4px below the caret. Previously the top edge was precomputed as `caretTop − estimatedHeight`, so custom styles that made rows shorter than 32px (reduced `dropdownItemPadding`, smaller fonts) left a gap between the dropdown and the input equal to the estimation error. Applies to both the suggestion dropdown and the date picker portal. Tested in `ElasticInput.browser.test.tsx` ("drop-up positioning" — flipped dropdown hugs the input when item styles are smaller than the 32px estimate).

**Post-render flip re-check:** because the height estimate counts a custom hint (`renderFieldHint`) as a single 32px row, the initial flip decision can be wrong for tall custom content. After the dropdown or date picker renders, a layout effect measures the actual height and calls `adjustFlippedPosition` (`domUtils.ts`): an unflipped dropdown that overflows the viewport bottom flips up if it fits above; a flipped dropdown that overflows the viewport top flips back down if it fits below. Direction only changes when the other side actually fits, so the correction converges and cannot oscillate. A flipped dropdown that would *also* fit below stays flipped (no jumping on borderline heights). Tests: `dropdownPosition.test.ts` (`adjustFlippedPosition`), `ElasticInput.browser.test.tsx` ("flips above when tall renderFieldHint content overflows below the viewport").
...
| Standalone calendar container | `ei-calendar` |
...
## 14. Standalone Calendar Component
