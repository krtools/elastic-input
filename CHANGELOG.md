...
- Rename `dropdown.mode` to `dropdown.open` and add callback support (`dropdown.mode` still works as deprecated fallback)
  ...
  **Migration:**
  ...
  - Rename `dropdown.mode` to `dropdown.open` (`mode` still works but is deprecated)

  `dropdown.open` now also accepts a callback `(ctx: DropdownOpenContext) => boolean | null` for programmatic control:
  - `true` — force the dropdown open
  - `false` — force it closed
  - `null` — let the engine decide
