...
- **Enter with the date picker open now submits** — It previously matched no key handler and fell through to the browser, which inserted a newline (e.g. with the caret at the end of a typed date range, where the picker is still open). It now closes the picker and submits, like Enter with the dropdown open and nothing selected.
...
- **Calendar wheel navigation** — `wheelNavigation` prop: mouse wheel over the calendar steps the view (month in days view, year in months view, decade in years view; down = forward). Page scroll is suppressed over the calendar. Off by default; `features.datePickerWheelNavigation` enables it in ElasticInput's date picker.
...
- **Tab no longer kicks focus out of the input while completions are loading** — Tab mid-partial is blocked while async `fields` or a suggestion fetch is still loading; once the data lands, the dropdown opens with the match auto-selected so the next Tab accepts it. Empty/whitespace positions and Shift+Tab always move focus, and a rejected fields loader clears the block.

### Fixes

- **Stale suggestions from a finished completion task are no longer acceptable** — Held results now clear when the completion task changes (e.g. field name → value), so Enter/Tab can't accept a leftover suggestion whose stale replace range would eat typed input. Same-field type-ahead is unchanged.
- **Pending dropdown-show frames can no longer resurrect a closed dropdown** — A position frame armed by an earlier keystroke could re-open dropdown state after a synchronous close, leaving an invisible state that swallowed Enter. Close paths now cancel the pending frame.
- **Arrow-selecting the spinner (or error/no-results item) no longer wipes the typed partial** — Inert items can no longer be accepted; the check is centralized in the new exported helpers `isInertSuggestion` / `isAcceptableSuggestion` / `hasPendingSuggestion`.
- **No more ghost dropdown after Enter-submitting during a fetch** — Submitting now cancels the pending fetch, debounce, and spinner timers, so superseded results can't pop the dropdown open over the search results.
- **Resilience against unstable `fields` identity** — Identity-only churn on the `fields` array (same elements, new array) no longer rebuilds the engine/validator or reprocesses the query every render, and internal engine/validator refs are lazily initialized. Inline literals that recreate the element objects still rebuild — see the README's *Prop Stability* section.
- **Prop identity stability documented** — New README *Prop Stability* section covering the three identity-sensitive props (`fields`, `colors`, function values inside `dropdown`) and what each costs per render when unstable; everything else is safe to write inline.
...
- **Reverse typing after a focus-state desync** — A blur processed while DOM focus never actually left the editor (e.g. window blur) left `isFocused` stale-false, so re-highlight rewrites skipped caret restore and every keystroke prepended at position 0 (`source` typed as `ecruos`). Caret save/restore is now gated on `document.activeElement` instead of the state flag.

### Demo

- **Stable `colors` identity** — The demo now memoizes its `ColorConfig`; a fresh object per render forced a full editor re-highlight on every keystroke and sustained the reverse-typing bug.
